...
export default function OperatorTokenDepositForm({ token, onCancel }: OperatorTokenDepositFormProps) {
  ...
  const submit = useCallback((event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity()) {
      ...
      void (async () => {
        try {
          await Operator.instance.deposit(token.data, token.data.parseAmount(amount), abortSignal);
          setSending(false);
          setAmount('');
          setValidated(false);
    ...
    <>
      { feedback === Feedback.SENDING ?
        <Alert variant="primary" className="m-3 font-monospace">
          <Spinner animation="border" size="sm" className="me-1" /> Depositing tokens...
        </Alert>

      : feedback === Feedback.SUCCESS ?
        <Alert variant="success" className="m-3 font-monospace">Tokens successfully deposited</Alert>

      : feedback === Feedback.INSUFFICIENT_FUNDS ?
        <Alert variant="danger" className="m-3 font-monospace">Your wallet does not have the amount of tokens requested</Alert>
      ...
      <Form noValidate validated={validated} onSubmit={submit} className="font-monospace">
        <Form.Group className="m-3" controlId="amount">
          ...
          <Form.Control type="number" required
            ...
            readOnly={sending} placeholder="Enter amount to deposit" />
        ...
        <div className="m-3">
          ...
          <Button variant="success" className="ms-3 font-monospace text-uppercase" type="submit" disabled={sending}>Deposit</Button>
        </div>
      </Form>
    </>
  );
